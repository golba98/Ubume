...
| `src/config/persistence.ts` | Loads and saves user-facing Codexa settings with normalization and safe fallback behavior. |
...
| `src/core/models/codexModelsCacheSeed.ts` | Seeds startup model capabilities from Codex and Codexa last-good cache files without blocking live discovery. |
...
Separate launch availability from in-Codexa routing. Persist workspace state through app data and test migrations and command construction.
...
| `src/core/providerLauncher/workspaceConfig.ts` | Loads, migrates, normalizes, and saves per-workspace provider configuration in Codexa app data. |
...
| `src/core/version/packageManager.ts` | Detects how Codexa was installed and derives the appropriate update command. |
...
Normalize paths consistently, keep mutable Codexa state outside projects, honor repository instructions, and test cross-platform and outside-workspace cases.
...
| `src/core/workspace/planStorage.ts` | Normalizes, saves, and reads reviewed plans from Codexa workspace data. |
...
| `src/core/workspace/scratchDir.ts` | Creates and prunes per-session `.codexa/scratch/<session>` folders where the Local agent keeps throwaway test/debug files. |
...
| `src/headless/execArgs.ts` | Parses codexa exec arguments, prompt policy, timing flags, and shared launch options. |
...
| `src/ui/chrome/TopHeader.tsx` | Renders responsive Codexa branding and workspace/auth/provider header content. |
...
| `src/ui/chrome/UpdateAvailableCard.tsx` | Renders an actionable notice when a newer Codexa version is available. |
...
| `src/ui/render/logoVariants.ts` | Chooses full, compact, or tiny Codexa branding for the available terminal dimensions. |
...
| `src/ui/timeline/StaticIntroItem.tsx` | Renders the scrollable Codexa intro/header as transcript content. |
