...
```bash
npm version 1.0.24 --no-git-tag-version
npm run gen-build-info
npm pkg get name version
```

Continue only after the printed version is `1.0.24`.
...
```bash
npm view @golba98/codexa@1.0.24 version --registry=https://registry.npmjs.org
npm install -g @golba98/codexa@1.0.24 --registry=https://registry.npmjs.org
codexa --version
```
...
```bash
git add -A
git commit -m "release: prepare Codexa v1.0.24"
git tag v1.0.24
git push origin agent/prepare-codexa-v1-0-22 --follow-tags
```
