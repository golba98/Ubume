...
```bash
npm version 1.0.28 --no-git-tag-version
npm run gen-build-info
npm pkg get name version
```

Continue only after the printed version is `1.0.28`.
...
```bash
npm view @golba98/codexa@1.0.28 version --registry=https://registry.npmjs.org
npm install -g @golba98/codexa@1.0.28 --registry=https://registry.npmjs.org
codexa --version
```
...
```bash
git add -A
git commit -m "release: prepare Codexa v1.0.28"
git tag v1.0.28
git push origin release/codexa-v1-0-28 --follow-tags
```
