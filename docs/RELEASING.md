...
Continue only after the printed version is `1.0.22`.
...
```bash
npm view @golba98/codexa@1.0.22 version --registry=https://registry.npmjs.org
npm install -g @golba98/codexa@1.0.22 --registry=https://registry.npmjs.org
codexa --version
```
...
```bash
git add -A
git commit -m "release: prepare Codexa v1.0.22"
git tag v1.0.22
