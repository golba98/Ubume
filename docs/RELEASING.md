...
Continue only after the printed version is `1.0.18`.
...
```bash
npm view @golba98/codexa@1.0.18 version --registry=https://registry.npmjs.org
npm install -g @golba98/codexa@1.0.18 --registry=https://registry.npmjs.org
codexa --version
```
...
```bash
git add -A
git commit -m "release: prepare Codexa v1.0.18"
git tag v1.0.18
git push origin agent/prepare-codexa-v1-0-18 --follow-tags
```
